...
@implementation MyPostImageTableViewCell
...
//    }else{
//        self.pageControl.numberOfPages = self.relevantMediaArray.count;
//        return self.relevantMediaArray.count;
//    }
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.messageType isEqualToString:@"MEDIA"]) {
        ...
        if (indexPath.row == 0) {
                ...
                [cell.postImageView sd_setImageWithURL:[NSURL URLWithString:[self.mediaDictionary valueForKey:@"standardResolutionURL"]] placeholderImage:[UIImage imageNamed:@"placeholder"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                }];
...
-(void)goToWebsiteWithProductDictionary:(NSDictionary *)dictionary{
    [self.delegate myPostGoToWebsite:dictionary];
}

-(void)addToCartWithProductDictionary:(NSDictionary *)dictionary{
    [self.delegate myPostAddTocart:dictionary];
}

//UIControl PageNation
